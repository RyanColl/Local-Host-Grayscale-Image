...
const grayScale = require("./grayscale.js").grayScale;
const port = 8080;
const readFile = require('./readWrite.js');
const htmlFile = path.join(__dirname, 'public', 'index.html');
const htmlFile1 = path.join(__dirname, 'public', 'index1.html');
...
let pngImg = link.slice(14)
    grayScale(path.join(__dirname, link), path.join(__dirname, 'public', 'grayscaled'))
        ...
        .then(() => {
            ...
            readFile.readFile(htmlFile, pngImg)
                .then(data => {
                    // for some reason my promise was returning the data i wanted in the err paramater, so i continued in there.
                }).catch(err => {
                    
                    readFile.writeFile(htmlFile1, err)
                        .catch(err => console.log(err))
                        .then(() => console.log('file written'))
                    fs.createReadStream(htmlFile1)
                        .on('open', function () {
                            res.statusCode = 200;
                            this.pipe(res);
                        })
                        .on('error', function () {
                            ...
                            res.statusCode = 404;
                            res.end('Error, did not enter png image');
                        })
                })
